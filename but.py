...
canvas.create_rectangle(0, 0, size_canvas_x, size_canvas_x, fill="white")
canvas.pack()

canvas1 = Canvas(tk, width=size_canvas_x, height=60, bd=0, highlightthickness=0)
canvas1.pack()
def on_closing():
...
def draw_table():
    ...
    for i in range(0, s_xy + 1):
        canvas.create_line(0, i * step, size_canvas_x, i * step)
    for i in range(0, s_xy + 1):
        canvas.create_line(i * step, 0, i * step, size_canvas_x)
...
f_bot.pack()
b_tic.pack(side=LEFT, padx=1)
b1.pack(side=LEFT)
b_tok.pack(side=LEFT, padx=1)
tk.update()
